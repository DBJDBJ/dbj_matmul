...
// mx as a pointer
#define MXY(M_, R_, C_) MXY_NOT_POINTER_((*M_), R_, C_)
...
static inline Matrix *matrix_addition(Matrix *res, const Matrix *a,
                                      const Matrix *b) {
  assert(res && a && b);
  FOR(i, res->rows)
  FOR(j, res->cols)
...
static inline Matrix *matrix_subtraction(Matrix *res, const Matrix *a,
                                         const Matrix *b) {
  assert(res && a && b);
  FOR(i, res->rows)
  FOR(j, res->cols)
...
static Matrix *matrix_ijk_matmul(Matrix *C, const Matrix *A, const Matrix *B) {
  ...
  FOR(i, A->rows) {
    FOR(j, B->cols) {
      MXY(C, i, j) = 0;
      FOR(k, A->cols) { MXY(C, i, j) += MXY(A, i, k) * MXY(B, k, j); }
    }
  }
  return C;
}
...
static void matrix_print(const char *prompt, Matrix *mx) {
  ...
  printf("%s\n", prompt);
  FOR(i, mx->rows) {
    FOR(j, mx->cols) { printf(" %5d ", MXY(mx, i, j)); }
    printf("\n");
  }
}

static void inline matrix_foreach(Matrix *mx,
                                  Matrix *(*callback)(Matrix *, unsigned,
                                                      unsigned)) {
  FOR(i, mx->rows) {
    FOR(j, mx->cols) { callback(mx, i, j); }
  }
}
