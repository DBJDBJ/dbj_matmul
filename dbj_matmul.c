...
https://godbolt.org/z/1KTE3PnEP
 ...
 Different compilers multiplied with different platforms multiplied with
selection of data types  yield a complex picture of benchmarking results.
...
static inline void *reset_test_result(app_data_type *app_data_) {
  return memset((void *)app_data_->r, 0,
                sizeof(dbj_matrix_data_type[DBJ_MX_R_ROWS * DBJ_MX_R_COLS]));
}
...
static void *matrix_arr_init(const unsigned rows_a, const unsigned cols_a,
                             dbj_matrix_data_type a[static rows_a][cols_a]) {
...
#define dbj_matrix_size_bytes(rows_, cols_, type_)                             \
  (rows_ * cols_ * sizeof(type_))

static void
dbj_matrix_transpose(const unsigned rows_m, const unsigned cols_m,
 ...
 */
static dbj_matrix_data_type *
matmul_mx_as_array_another(const size_t a_rows, const size_t a_cols,
                           const size_t b_cols, dbj_matrix_data_type *a,
                           dbj_matrix_data_type *b, dbj_matrix_data_type *c,
...
// this is VMT based
static void *
matmul_sdot8(const unsigned a_rows, const unsigned a_cols,
             const unsigned b_cols,
             dbj_matrix_data_type a[static a_rows][a_cols],
             dbj_matrix_data_type b[static a_cols][b_cols],
             dbj_matrix_data_type m[static a_rows][b_cols],
             // allocated space for transposed b
             dbj_matrix_data_type bT[static b_cols][a_cols]) { // orinteering
...
static void *
matmul_sdot_1(const unsigned a_rows, const unsigned a_cols,
              const unsigned b_cols,
              dbj_matrix_data_type a[static a_rows][a_cols],
              dbj_matrix_data_type b[static a_cols][b_cols],
              dbj_matrix_data_type m[static a_rows][b_cols],
              // allocated space for transposed b
              dbj_matrix_data_type bT[static b_cols][a_cols]) { // orienteering
  // const unsigned b_rows  = a_cols;
  // const unsigned bt_rows = b_cols;
  // const unsigned bt_cols = b_rows ;
...
static void mohapatra(const unsigned Arows, const unsigned Acols, /* == Brows */
  ...
  FOR(j, Acols) {
    double sum_1 = 0;
    FOR(i, Arows) { sum_1 = sum_1 * (pow(10, P)) + B[Arows - 1 - i][j]; }
    D[j] = sum_1;
  }
...
#define DBJ_APP_KIND "TESTING"
  ...
  dbj_matrix_transpose(app_data->rows_b, app_data->cols_b, (void *)app_data->b,
                       (void *)app_data->bT);
#endif // DBJ_MX_ALREADY_TRANSPOSED

  const float size_a = dbj_matrix_size_bytes(app_data->rows_a, app_data->cols_a,
                                             dbj_matrix_data_type) /
                       1024.0f;
  const float size_b = dbj_matrix_size_bytes(app_data->rows_b, app_data->cols_b,
                                             dbj_matrix_data_type) /
                       1024.0f;
  const float size_bT =
      dbj_matrix_size_bytes(app_data->rows_bT, app_data->cols_bT,
                            dbj_matrix_data_type) /
      1024.0f;
  const float size_r = dbj_matrix_size_bytes(app_data->rows_r, app_data->cols_r,
                                             dbj_matrix_data_type) /
                       1024.0f;

  fprintf(
      stderr,
      "\n\n" DBJ_VT_RED " " DBJ_APP_KIND " " DBJ_VT_RESET
      ...
      DBJ_BUILD_TIMESTAMP, app_data->rows_a, app_data->cols_a,
      dbj_matrix_data_type_name, size_a, app_data->rows_b, app_data->cols_b,
      dbj_matrix_data_type_name, size_b, app_data->rows_bT, app_data->cols_bT,
      dbj_matrix_data_type_name, size_bT, app_data->rows_r, app_data->cols_r,
      dbj_matrix_data_type_name, size_r);
#undef DBJ_APP_KIND
}
...
UBENCH(matmul, transpose_and_sdot8) {
  matmul_sdot8(DBJ_MX_A_ROWS, DBJ_MX_A_COLS, DBJ_MX_B_COLS, app_data->a,
               app_data->b, app_data->r, app_data->bT);
}

UBENCH(matmul, mx_as_array_another) {
  matmul_mx_as_array_another(DBJ_MX_A_ROWS, DBJ_MX_A_COLS, DBJ_MX_B_COLS,
                             (void *)app_data->a, (void *)app_data->b,
                             (void *)app_data->r, (void *)app_data->bT);
}

UBENCH(matmul, mx_as_array) {
  matmul_mx_as_array(DBJ_MX_A_ROWS, DBJ_MX_A_COLS, DBJ_MX_B_COLS,
                     (void *)app_data->a, (void *)app_data->b,
                     (void *)app_data->r);
}
